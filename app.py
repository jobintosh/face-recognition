...
# load from env MYSQL_HOST first, if not found, use default value 'mariadb'
...
def recognize_face_from_img_path(img_path):
    ...
    for face in recognized_faces:
        ...
        if label == -1:
            label = "UNKNOWN"  # Assign "UNKNOWN" label to unrecognized faces
    ...
    # Assign the label and confidence of the first recognized face
...
def send_password_reset_email(email, temp_password):
    ...
    try:
        ...
        smtp_server.login('Replace with your email', 'Replace with your password')
        smtp_server.sendmail('Replace with your email',
                             [email], msg.as_string())
        smtp_server.quit()
...
def fr_page():
    if 'user_id' in session:
        ...
        if has_active_subscription(user_id):
            ...
            return render_template('access_denied.html')  # Use a separate template for access denied page
    else:
        return 'You are not logged in. <a href="/login">Login</a>'
...
def edit(person_id):
    if 'user_id' in session:
        ...
        # Check subscription status here and set the subscription_status variable
...
def success():
    # Retrieve user_id from the user session
    ...
    if user_id is not None:
        try:
            ...
            if user_data:
                ...
                # Insert the data into the 'subscriptions' table
        ...
        # The user is not logged in or the user_id is not available in the session
        return redirect(url_for('login'))
...
def check():
    try:
        ...
        # SQL query to check the condition and retrieve locker_no
        query = """
            SELECT p.locker_no
...
def check_subscription():
    ...
    if user_id is not None:
        # Fetch the current subscription data from the database
        query = "SELECT start_date, end_date FROM subscriptions WHERE user_id = %s"
        mycursor.execute(query, (user_id,))
        subscription = mycursor.fetchone()

        # Clear the results from the cursor
        mycursor.fetchall()

        if subscription:
            start_date = subscription[0]  # Start date from the database
            end_date = subscription[1]  # End date from the database

            today = datetime.now().date()

            # Check if the subscription is active today
